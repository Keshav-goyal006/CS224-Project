...
volatile uint32_t* const SWITCHES      = (uint32_t*)0x00006000;
...
volatile uint32_t* const WARM_RESET_CLEAR = (uint32_t*)0x00006014;
volatile uint8_t* const image_pixels   = (volatile uint8_t*)0x00001000;

static void acknowledge_warm_reset(void) {
    *WARM_RESET_CLEAR = 1u;
}

static void show_selected_filter(void) {
    *LEDS = *SWITCHES & 0xFu;
}

static void run_filter_once(void) {
